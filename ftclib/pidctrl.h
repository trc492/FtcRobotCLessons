...
// Constants.
//
#define PIDCTRLO_INVERSE        0x0001
#define PIDCTRLO_ABS_SETPT      0x0002
#define PIDCTRLO_NO_OSCILLATE   0x0004
#define PIDCTRLO_INTEGRATE      0x0008
...
//
/**
 *  This macro checks if the two PID controllers are the same.
 *
 *  @param p1 Points to the first PID controller.
 *  @param p2 Points to the second PID controller.
 *
 *  @return Returns true if the two PID controllers are the same.
 */
#define PIDCtrlCheck(p1,p2)     (&(p1) == &(p2))
 ...
 *         PIDCTRLO_INVERSE - Setpoint is inverse.
    ...
    )
{
    ...
    if (pidCtrl.options & PIDCTRLO_INVERSE)
    {
        error = -error;
    }
